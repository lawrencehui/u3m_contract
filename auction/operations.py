...
def placeBid(client: AlgodClient, appID: int, bidder: Account, bidAmount: int) -> None:
    ...
    if any(appGlobalState[b"bid_account"]):
        ...
        print(
            f'checking appGlobalState[b"bid_account"]: {appGlobalState[b"bid_account"]}')
        print(
            f'encoded: {encoding.encode_address(appGlobalState[b"bid_account"]) }')
...
def closeAuction(client: AlgodClient, appID: int, closer: Account):
    ...
    print(f"nftID: {nftID}")
    print(f'seller {appGlobalState[b"seller"]} ')
    print(
        f'bid_account {encoding.encode_address(appGlobalState[b"bid_account"])} ')

    # seller_address = appGlobalState[b"seller"].decode("utf-8")

    print(encoding.encode_address(appGlobalState[b"seller"]))

    accounts: List[str] = [encoding.encode_address(appGlobalState[b"seller"])]
    # accounts: List[str] = [seller_address]

    if any(appGlobalState[b"bid_account"]):
        # if "bid_account" is not the zero address
        print('evaluate bid_account as true')
        accounts.append(encoding.encode_address(
            appGlobalState[b"bid_account"]))
    else:
        print("bid_account is zero address")
